...
class module_archive extends module_base
{
  function view()
  {
    global $WEB_APP;
    ...
    if (isset($_POST["submit_button"])) {
      if (isset($_POST['date_from']) and $_POST['date_to'] and $_POST['group']) {
        $result = archive_results($groups_to_archive, $_POST['date_from'], $_POST['date_to']);
    ...
    $WEB_APP['title'] = text('txt_archive_results');
    $WEB_APP['submit_title'] = text('txt_archive');
    $WEB_APP['form_title'] = text('txt_archive_results');
    $WEB_APP['scripts'][] = 'moment-with-locales.min.js';
    $WEB_APP['scripts'][] = 'bootstrap-datetimepicker.min.js';
    $WEB_APP['view']->display('form_page.tpl', text('txt_archive_results'));
  }
}